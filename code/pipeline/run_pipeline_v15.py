"""
Pipeline v15: Restormer → OMDNet (PIL 1440) → Sharpen(0.5) → Real-ESRGAN x4
For text-heavy images — no CLAHE, no DiffBIR.
"""
import glob
import sys
import os
...
def run_step1_restormer(image_ids, output_dir):
    ...
    for img_id in image_ids:
        ...
        for prev_dir in prev_sources:
            ...
            if os.path.exists(src):
                ...
                print(f'  [{img_id}] Reused from {os.path.basename(os.path.dirname(prev_dir))}')
                break
    ...
    remaining = [f for f in collect_photos(image_ids)
                 if not os.path.exists(os.path.join(output_dir, f'{get_image_id(f)}.png'))]
...
def run_step2_omdnet(image_ids, input_dir, output_dir, target_max_dim=1440):
    print('\n' + '=' * 60)
    print(f'STEP 2: OMDNet (PIL resize {target_max_dim})')
    print('=' * 60)
    os.makedirs(output_dir, exist_ok=True)
...
def run_step3_sharpen(image_ids, input_dir, output_dir, amount=0.5, radius=1.0):
    print('\n' + '=' * 60)
    print(f'STEP 3: Sharpen (amount={amount})')
    print('=' * 60)
    os.makedirs(output_dir, exist_ok=True)
...
def run_step4_realesrgan(image_ids, input_dir, output_dir, outscale=4):
    print('\n' + '=' * 60)
    print(f'STEP 4: Real-ESRGAN x{outscale}')
    print('=' * 60)
    os.makedirs(output_dir, exist_ok=True)
...
def main():
    ...
    parser.add_argument('--images', type=str, default='02,03,10,11,12')
    parser.add_argument('--start_from', type=int, default=1)
    args = parser.parse_args()

    image_ids = [x.strip().zfill(2) for x in args.images.split(',')]
    results_dir = os.path.join(BASE_DIR, 'results', 'pipeline_v15_text')
    step1_dir = os.path.join(results_dir, 'step1_restormer')
    step2_dir = os.path.join(results_dir, 'step2_omdnet')
    step3_dir = os.path.join(results_dir, 'step3_sharpen')
    step4_dir = os.path.join(results_dir, 'step4_realesrgan')

    print('Pipeline v15: Restormer → OMDNet (PIL 1440) → Sharpen(0.5) → ESRGAN x4')
    ...
    if args.start_from <= 3:
        run_step3_sharpen(image_ids, step2_dir, step3_dir)

    if args.start_from <= 4:
        run_step4_realesrgan(image_ids, step3_dir, step4_dir)

    elapsed = time.time() - t0
    print(f'\nPipeline v15 done in {elapsed / 60:.1f} min.')
    print(f'Results: {step4_dir}')
