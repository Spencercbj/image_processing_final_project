"""
Pipeline v18: PIL resize 1440 → Restormer → OMDNet → NLMeans denoise → Sharpen(0.5) → ESRGAN x4
All 15 images. Face images (01,03,10,14,15) use GFPGAN face enhancement in ESRGAN step.
"""
import glob
import sys
import os
...
def run_step2_restormer(image_ids, input_dir, output_dir):
    print('\n' + '=' * 60)
    print('STEP 2: Restormer (on 1440 images)')
    print('=' * 60)
    os.makedirs(output_dir, exist_ok=True)
...
def run_step4_denoise(image_ids, input_dir, output_dir, h=6, h_color=6,
                      template_ws=7, search_ws=21):
    print('\n' + '=' * 60)
    print(f'STEP 4: NLMeans Denoise (h={h}, hColor={h_color})')
    print('=' * 60)
    os.makedirs(output_dir, exist_ok=True)
    ...
    for fpath in files:
        ...
        if os.path.exists(out_path):
            ...
            continue
        img = cv2.imread(fpath)
        denoised = cv2.fastNlMeansDenoisingColored(img, None, h, h_color,
                                                    template_ws, search_ws)
...
def run_step6_realesrgan(image_ids, input_dir, output_dir, outscale=4):
    """Run ESRGAN in two batches: face images with --face_enhance, others without."""
    print('\n' + '=' * 60)
    print(f'STEP 6: Real-ESRGAN x{outscale} (face_enhance for {sorted(FACE_IDS)})')
    print('=' * 60)
    os.makedirs(output_dir, exist_ok=True)
    ...
    face_files = [f for f in files if get_image_id(f) in FACE_IDS]
    noface_files = [f for f in files if get_image_id(f) not in FACE_IDS]

    def run_batch(batch_files, face_enhance, label):
        if not batch_files:
            return
        tmp_in = os.path.join(output_dir, f'_tmp_esrgan_{label}')
        ...
        fe_str = ' + face_enhance' if face_enhance else ''
        print(f'  Running ESRGAN{fe_str} on {ids} ...')
        result = subprocess.run(cmd, cwd=ESRGAN_ROOT)
        shutil.rmtree(tmp_in, ignore_errors=True)
        if result.returncode != 0:
            print(f'  WARNING: ESRGAN {label} batch had errors')
        else:
            print(f'  {label} batch done.')

    run_batch(face_files, face_enhance=True, label='face')
    run_batch(noface_files, face_enhance=False, label='noface')
...
def main():
    ...
    results_dir = os.path.join(BASE_DIR, 'results', 'pipeline_v18')
    ...
    step4_dir = os.path.join(results_dir, 'step4_denoise')
    step5_dir = os.path.join(results_dir, 'step5_sharpen')
    step6_dir = os.path.join(results_dir, 'step6_realesrgan')

    print('Pipeline v18: resize 1440 -> Restormer -> OMDNet -> NLMeans -> Sharpen -> ESRGAN x4')
    print(f'Images: {image_ids}')
    print(f'Face enhance: {sorted(FACE_IDS)}')
    t0 = time.time()
    ...
    if args.start_from <= 4:
        run_step4_denoise(image_ids, step3_dir, step4_dir)
    ...
    print(f'\nPipeline v18 done in {elapsed / 60:.1f} min.')
